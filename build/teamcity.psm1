...
function TeamCity-FailBuild([string]$reason) {
    Write-Output "##teamcity[buildStatus status='FAILURE' text='$reason']"
}
...
function TeamCity-ImportNUnitResult([string]$path) {
    Write-Output "##teamcity[importData type='nunit' path='$path']"
}
