...
(function(window, document, $, ko) {
  ...
  bo.Uri = (function() {
    ...
    function Uri(uri, options) {
      ...
      this.query = (_ref2 = anchor.search) != null ? _ref2.substring(1) : void 0;
  ...
  validateObservable = function(observableValue, model) {
    ...
    return observableValue.isClientValid(errors.length === 0);
  };
  validateModel = function(model, parent) {
    ...
    model.validate = function() {
      if (!model.validated()) {
        ko.computed(function() {
          return model.isClientValid(validateModel(model));
        });
  ...
  ko.extenders.validationRules = function(target, validationRules) {
    ...
    target.validate = _.once(function(parentModel) {
      var doValidate;
      doValidate = function() {
        target.validated(true);
        return validateObservable(target, parentModel);
      };
  ...
  rules = {
    ...
    minDate: {
      ...
      message: function(options) {
        return "This field must be on after " + options[0];
      }
    },
    inFuture: {
      validator: function(value, options) {
    ...
    requiredIf: {
      validator: function(value, options, model) {
        ...
        if (options.value === void 0 && options.property === void 0) {
          throw new Error("You need to provide either a property or a value.");
        }
        if ((options.property != null) && !_.has(model, options.property)) {
    ...
    equalTo: {
      validator: function(value, options, model) {
        if (options.value != null) {
          return (emptyValue(value)) || (value === ko.utils.unwrapObservable(options.value));
        } else {
          return (emptyValue(value)) || (value === ko.utils.unwrapObservable(model[options]));
        }
      },
      message: function(options) {
        if (options.value != null) {
          return "This field must be equal to " + options.value + ".";
        } else {
  ...
  bo.DataSource = (function() {
    function DataSource(options) {
      ...
      this._sortByDetails.subscribe(__bind(function(newValue) {
        var normalised;
        ...
        return this._sortByAsString(normalised);
      }, this));
      this.sortBy = ko.computed({
        ...
        write: __bind(function(value) {
          ...
          return this._sortByDetails(properties);
        }, this)
      });
      this.items = ko.computed(__bind(function() {
        if ((this._sortByDetails() != null) && !this._serverPagingEnabled) {
          return this._loadedItems().sort(__bind(function(a, b) {
            var p, _i, _len, _ref;
            _ref = this._sortByDetails();
            for (_i = 0, _len = _ref.length; _i < _len; _i++) {
              p = _ref[_i];
      ...
      if (this.options.searchParameters != null) {
        ...
        this.searchParameters.subscribe(__bind(function() {
    ...
    DataSource.prototype._setupPaging = function() {
      ...
      this.pageItems = ko.computed(__bind(function() {
        var end, start;
        if (this._clientPagingEnabled && this._serverPagingEnabled) {
          start = ((this.pageNumber() - 1) % this.clientPagesPerServerPage) * this.pageSize();
          end = start + this.pageSize();
          return this.items().slice(start, end);
        } else if (this._clientPagingEnabled) {
      ...
      this.pageCount = ko.computed(__bind(function() {
        if (this.totalCount()) {
      ...
      this.isFirstPage = ko.computed(__bind(function() {
        return this.pageNumber() === 1;
      }, this));
      this.isLastPage = ko.computed(__bind(function() {
        return this.pageNumber() === this.pageCount() || this.pageCount() === 0;
      }, this));
      if (this.options.serverPaging) {
        this.pageNumber.subscribe(__bind(function() {
          return this._doLoad();
        }, this));
        return this.sortBy.subscribe(__bind(function() {
          return this._doLoad();
        }, this));
      }
    };
    DataSource.prototype._doLoad = function() {
      ...
      this.isLoading(true);
      return this.options.provider(loadOptions, (__bind(function(loadedData) {
        this._setData(loadedData);
        this._lastProviderOptions = loadOptions;
  ...
  Route = (function() {
    ...
    function Route(name, url, callback, options) {
      ...
      this.requiredParams = [];
      this.paramNames = [];
      routeDefinitionAsRegex = this.url.replace(paramRegex, __bind(function(_, mode, name) {
    ...
    Route.prototype.match = function(path) {
      var index, matches, name, params, _len, _ref;
    ...
    Route.prototype.buildUrl = function(parameters) {
      ...
      if (this._allRequiredParametersPresent(parameters)) {
        return this.url.replace(paramRegex, __bind(function(_, mode, name) {
  ...
  bo.routing.Router = (function() {
    function Router() {
      this.routes = {};
      bo.bus.subscribe('urlChanged:external', __bind(function(msg) {
        var matchedRoute;
        matchedRoute = this.getRouteFromUrl(msg.url);
        if (matchedRoute === void 0) {
          return bo.bus.publish('routeNotFound', {
            url: msg.url
          });
        } else {
          return this._doNavigate(msg.path, matchedRoute.route, matchedRoute.parameters);
        }
      }, this));
    }
  ...
  tagBindingProvider = function() {
    var findTagCompatibleBindingHandlerName, mergeAllAttributes, processBindingHandlerTagDefinition, realBindingProvider;
    ...
    findTagCompatibleBindingHandlerName = function(node) {
      var tagName;
      if (node.tagHandler != null) {
        ...
        if (tagName != null) {
          return _.find(_.keys(koBindingHandlers), function(key) {
            var bindingHandler;
    ...
    this.preprocessNode = function(node) {
      var nodeReplacement, tagBindingHandler, tagBindingHandlerName, _ref;
      tagBindingHandlerName = findTagCompatibleBindingHandlerName(node);
      tagBindingHandler = koBindingHandlers[tagBindingHandlerName];
      if (tagBindingHandlerName && (((_ref = tagBindingHandler.tag) != null ? _ref.replacedWith : void 0) != null)) {
    ...
    this.getBindings = function(node, bindingContext) {
      var existingBindings, options, optionsAttribute, tagBindingHandlerName;
      existingBindings = realBindingProvider.getBindings(node, bindingContext);
      tagBindingHandlerName = findTagCompatibleBindingHandlerName(node);
      if (tagBindingHandlerName !== void 0) {
