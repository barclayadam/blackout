...
messaging = (_ref = bo.messaging) != null ? _ref : bo.messaging = {};
...
*/
messaging.command = function (commandName, values) {
    if (values == null) {
        values = {};
    }
    ...
    return bo.ajax
            .url(messaging.commandUrlTemplate.replace("{name}", commandName))
            .data(values)
            .post();
};
...
messaging.Command = (function () {
    ...
    */
    ...
    Command.prototype.execute = function () {
        ...
        if (this.isValid()) {
            return messaging.command(this.__name, this);
        } else {
