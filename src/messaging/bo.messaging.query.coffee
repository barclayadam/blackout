...
#
# The URL template that is used is defined by `bo.messaging.queryUrlTemplate`, with two
...
messaging.query = (queryName, options = {}) ->
    bo.log.info "Executing query '#{queryName}'."

    bo.ajax.url(messaging.queryUrlTemplate.replace("{values}", encodeURIComponent ko.toJSON options).replace("{name}", queryName))
        .get()